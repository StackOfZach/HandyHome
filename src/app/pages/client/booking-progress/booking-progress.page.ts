...
export class BookingProgressPage implements OnInit, OnDestroy {
  ...
  workerCurrentLocation: { lat: number; lng: number; timestamp?: any } | null =
    null;
  ...
  private handleLocationTrackingUpdate(previousStatus?: string) {
    ...
    console.log('Checking if should start worker location tracking:', {
      ...
      previousStatus,
      shouldStart:
        currentStatus === 'on-the-way' && previousStatus !== 'on-the-way',
    });
    ...
    if (currentStatus === 'on-the-way' && !this.workerLocationSubscription) {
      console.log(
        'Starting worker location tracking - status is already on-the-way'
      );
      this.startWorkerLocationTracking();
    }
  ...
  async loadWorkerProfile() {
    ...
    try {
      ...
      console.log(
        'Worker serviceWithPricing:',
        (workerProfile as any)?.serviceWithPricing
      );
      console.log('Booking neededService:', this.booking.neededService);
      console.log(
        'Booking specificService:',
        (this.booking as any).specificService
      );
      ...
      if (workerProfile) {
        this.worker = {
          ...
          photoUrl:
            workerProfile.profilePhotoUrl || '/assets/default-avatar.png',
        ...
        if (this.booking?.status === 'on-the-way') {
          console.log(
            'Booking status is on-the-way, starting location tracking...'
          );
          setTimeout(() => {
            this.startWorkerLocationTracking();
  ...
  async cancelBooking() {
    ...
    if (warningMessage) {
      message =
        warningMessage +
        'Cancelling now may result in cancellation fees. Are you sure you want to proceed?';
    }
  ...
  async loadWorkerPricing() {
    ...
    try {
      ...
      if (
        this.worker.serviceWithPricing &&
        this.worker.serviceWithPricing.length > 0
      ) {
        ...
        console.log(
          ...
          this.worker.serviceWithPricing.map((c) => c.categoryName)
        );

        // First, try to find the matching category (main service) - more flexible matching
        matchedCategory =
          this.worker.serviceWithPricing.find(
        ...
        if (matchedCategory) {
          ...
          if (specificService) {
            ...
            foundPricing =
              matchedCategory.subServices.find(
                (subService: SubServicePrice) => {
                  const subServiceLower =
                    subService.subServiceName.toLowerCase();
        ...
        if (!foundPricing && specificService) {
          for (const category of this.worker.serviceWithPricing) {
            const subServiceMatch = category.subServices.find(
              (subService) =>
                subService.subServiceName
                  .toLowerCase()
                  .includes(specificService.toLowerCase()) ||
                specificService
                  .toLowerCase()
                  .includes(subService.subServiceName.toLowerCase())
            );
            if (subServiceMatch) {
      ...
      if (!this.workerPricing) {
        console.log(
          'No worker pricing found in profile, trying direct fetch from workers collection...'
        );
      ...
      if (!this.workerPricing) {
        console.log(
          'No worker pricing found anywhere, creating mock data for testing'
        );
  ...
  async findPricingInData(
    ...
    console.log(
      'Searching in direct data - Available categories:',
    ...
    matchedCategory =
      serviceWithPricing.find((category: ServiceWithPricing) => {
        ...
        console.log(
          `Direct search - Comparing category "${categoryLower}" with service "${serviceLower}"`
        );
  ...
  isNewWorker(): boolean {
    ...
    return (
      this.worker.rating === 0 ||
      this.worker.rating === null ||
      this.worker.reviewCount === 0 ||
      this.worker.reviewCount === null
    );
  }
  ...
  async startWorkerLocationTracking() {
    console.log('startWorkerLocationTracking called with:', {
      ...
      shouldProceed: !!(
        this.booking?.workerId && this.booking.status === 'on-the-way'
      ),
    });
    ...
    if (this.booking.status !== 'on-the-way') {
      console.log(
        'Status is not on-the-way, current status:',
        this.booking.status
      );
      return;
    }
    ...
    console.log(
      'Starting worker current location tracking for worker:',
      this.booking.workerId
    );
          ...
          async (doc) => {
            if (doc.exists()) {
              ...
              if (currentLocation) {
                ...
                if (
                  currentLocation._lat !== undefined &&
                  currentLocation._long !== undefined
                ) {
                ...
                // Handle latitude/longitude naming
                  ...
                  console.log('Found latitude/longitude location:', {
                    lat,
                    lng,
                  });
                } else {
                  console.log(
                    'No valid coordinates found in currentLocation:',
                    currentLocation
                  );
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data && data.display_name) {
          ...
          this.workerCurrentAddress = `Near ${lat.toFixed(4)}, ${lng.toFixed(
            4
          )}`;
        }
      } else {
  ...
  async getAddressFromCoordinatesAlternative(
    lat: number,
    lng: number
  ): Promise<void> {
    try {
      ...
      if (response.ok) {
        ...
        if (data && data.display_name) {
          ...
          console.log(
            'Worker current address (Nominatim):',
            this.workerCurrentAddress
          );
        } else {
          this.workerCurrentAddress = `Near ${lat.toFixed(4)}, ${lng.toFixed(
    ...
    if (this.booking.scheduleDate) {
      try {
        ...
        if (
          ...
          typeof this.booking.scheduleDate.toDate === 'function'
        ) {
          scheduleDate = this.booking.scheduleDate.toDate();
  ...
  getCancellationMessage(): string {
    ...
    if (this.booking.status === 'pending') {
      ...
      ['on-the-way', 'service-started'].includes(this.booking.status)
    ) {
      return 'Cancel Booking';
    }
      ...
      if (workerDoc.exists()) {
        ...
        if (workerData['currentLocation']) {
          ...
          // Handle latitude/longitude naming
            ...
            lng = loc.longitude;
            console.log(
              'Valid latitude/longitude coordinates found:',
              lat,
              lng
            );
          } else {
      ...
      case 'completed':
        ...
        if (this.showPaymentModal) {
          ...
          setTimeout(async () => {
            const toast = await this.toastController.create({
              message:
                'Payment confirmed by worker! Thank you for using HandyHome.',
              duration: 4000,
              color: 'success',
            });
            await toast.present();
