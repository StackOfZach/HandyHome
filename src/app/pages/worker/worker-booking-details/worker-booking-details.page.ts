...
export class WorkerBookingDetailsPage implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    ...
    this.isQuickBooking =
      this.route.snapshot.queryParamMap.get('type') === 'quick';
  ...
  async loadBookingDetails() {
    try {
      ...
      const primaryCollection = this.isQuickBooking
        ? 'quickbookings'
        : 'bookings';
      ...
      if (bookingDoc.exists()) {
        ...
        const secondaryCollection = this.isQuickBooking
          ? 'bookings'
          : 'quickbookings';
      ...
      if (bookingDoc && bookingDoc.exists()) {
        ...
        if (this.booking.status === 'completed') {
          console.warn(
            'Booking status is completed. Checking if this is correct...'
          );
          ...
          if (!hasCompletionData) {
            console.error(
              '❌ BOOKING STATUS ERROR: Status is completed but no completion data found!'
            );
        ...
        if (this.booking.clientId) {
          ...
          console.log(
            'No clientId found, checking for embedded client details...'
          );
        ...
        if (
          ...
          !this.booking.clientDetails.name ||
          this.booking.clientDetails.name === 'Client'
        ) {
        ...
        if (
          ...
          this.booking.jobTimer?.startTime
        ) {
          this.startJobTimer();
        }
      } else {
        console.error(
          `Booking ${this.bookingId} not found in either collection`
        );
  ...
  async loadClientDetails() {
    ...
    try {
      ...
      console.log(
        'Fetching client document from:',
        `users/${this.booking.clientId}`
      );
      ...
      if (clientDoc.exists()) {
        ...
        const name =
          ...
          'Unknown Client';
        const phone =
          clientData['phoneNumber'] || clientData['phone'] || 'No phone';
        ...
        this.booking.clientDetails = {
          ...
          email: email,
          profilePicture:
            clientData['profilePicture'] || clientData['photoURL'] || '',
          address: clientData['address'] || 'No address',
          verified:
            clientData['verified'] || clientData['emailVerified'] || false,
        };
        ...
        console.warn(
          'Client document not found for ID:',
          this.booking.clientId
        );
        console.warn(
          'Document path attempted:',
          `users/${this.booking.clientId}`
        );
  ...
  setupRealtimeListener() {
    ...
    console.log(
      `Setting up real-time listener for ${collectionName} collection`
    );
    ...
    const unsubscribe = onSnapshot(bookingRef, (doc) => {
      if (doc.exists()) {
        ...
        if (
          ...
          existingClientDetails.name !== 'Loading...' &&
          existingClientDetails.name !== 'Client'
        ) {
          console.log(
            'Preserving existing client details:',
            existingClientDetails
          );
  ...
  async sendMessage() {
    ...
    const workerName =
      this.booking.workerDetails?.name || 'Your HandyHome Worker';
    ...
    try {
      ...
      const smsUrl = `sms:${
        this.booking.clientDetails.phone
      ...
      this.showToast(
        'Could not open SMS app. Please try calling instead.',
        'danger'
      );
    }
  }
  ...
  async startJob() {
    ...
    console.log(
      'Collection type:',
      this.isQuickBooking ? 'quickbookings' : 'bookings'
    );
  ...
  async submitCompletion() {
    ...
    try {
      ...
      if (dynamicPricing && duration > 0) {
        ...
        console.log(
          '✅ Ensured duration is set in dynamicPricing:',
          dynamicPricing.duration
        );
      }
      ...
      console.log(
        'Original duration passed to calculateDynamicPricing:',
        duration
      );
  ...
  private startJobTimer() {
    ...
    this.jobTimer = setInterval(() => {
      ...
      this.elapsedTime = `${hours.toString().padStart(2, '0')}:${minutes
        .toString()
  ...
  async calculateDynamicPricing(durationInSeconds: number): Promise<any> {
    ...
    if (!durationInSeconds || durationInSeconds <= 0) {
      console.error(
        '❌ Invalid duration provided to calculateDynamicPricing:',
        durationInSeconds
      );
      return null;
    }
    ...
    try {
      ...
      const basePrice =
        subService.price || this.booking.pricing?.basePrice || 0;
      ...
      if (pricingType === 'hourly' || pricingType === '/hr') {
        ...
        console.log(
          `Hourly calculation: ${roundedHours} hours × ₱${basePrice} = ₱${calculatedBasePrice}`
        );
        ...
        console.log(
          `Daily calculation: ${roundedDays} days × ₱${basePrice} = ₱${calculatedBasePrice}`
        );
      } else {
  ...
  async fetchServiceLocationAddress(lat: number, lng: number): Promise<void> {
    ...
    try {
      console.log(
        'Fetching precise service location address from Nominatim...'
      );
      ...
      if (data && data.address) {
        ...
        if (barangay) {
          structuredAddress +=
            (structuredAddress ? ', ' : '') + `Barangay ${barangay}`;
        }
        if (city) {
        ...
        console.log(
          'Parsed service location details:',
          this.serviceLocationDetails
        );
